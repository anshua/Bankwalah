...
<button type=\"button\" class=\"btn btn-warning btn-xs\" onclick=\"printDiv('printbranchrecord')\">PRINT BRANCH RECORD</button><a href=\"clerk-branch-edit.php?id=".$row["BrID"]."\"><button type=\"button\" class=\"btn btn-warning btn-xs\" style=\"margin-left:20px;\">EDIT</button></a>";
	}
}
else {
...
<table class=\"table table-striped table-bordered\">
	...
	while($row = $result->fetch_assoc()) {
		echo "<tr>
		<td>".$row["EmpID"]."</td>
		<td><a href=\"clerk-employee-view.php?id=".$row["EmpID"]."\">".$row["EmpName"]."</a></td>
		<td>".$row["EmpPhone"]."</td>
		<td>".$row["EmpEmail"]."</td>
		<td>".$row["EmpRole"]."</td>
